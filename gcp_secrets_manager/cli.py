...
Ejemplos de uso:
  ...
  gcp-secrets-cli -p mi-proyecto --demo
    ...
    parser.add_argument(
        "--project", "-p", help="ID del proyecto GCP (o usa GCP_PROJECT_ID env var)"
    )
    parser.add_argument(
        "--verify-auth", action="store_true", help="Verificar autenticación con GCP"
    )
    parser.add_argument(
        "--cache-ttl", type=int, default=300, help="TTL del caché en segundos (default: 300)"
    )

    # Comandos para obtener secretos
    secret_group = parser.add_argument_group("comandos de secretos")
    secret_group.add_argument(
        "--get-api-key", metavar="SERVICE", help="Obtener API key para un servicio"
    )
    secret_group.add_argument(
        "--get-api-creds", metavar="SERVICE", help="Obtener todas las credenciales API"
    )
    secret_group.add_argument(
        "--get-db-creds", metavar="DB_NAME", help="Obtener credenciales de base de datos"
    )
    secret_group.add_argument(
        "--get-auth-creds", metavar="SERVICE", help="Obtener credenciales de autenticación"
    )
    secret_group.add_argument(
        "--get-custom", metavar="SECRET_NAME", help="Obtener un secreto personalizado"
    )

    # Comandos de utilidad
    util_group = parser.add_argument_group("comandos de utilidad")
    util_group.add_argument("--cache-status", action="store_true", help="Ver estado del caché")
    util_group.add_argument("--clear-cache", action="store_true", help="Limpiar todo el caché")
    util_group.add_argument(
        "--list-legacy", action="store_true", help="Listar secretos legacy disponibles"
    )
    util_group.add_argument("--demo", action="store_true", help="Ejecutar demostración completa")

    # Formato de salida
    parser.add_argument("--json", action="store_true", help="Salida en formato JSON")
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Solo mostrar valores, sin mensajes adicionales"
    )
...
def get_project_id(args) -> Optional[str]:
    ...
    project_id = (
        args.project or os.environ.get("GCP_PROJECT_ID") or os.environ.get("GOOGLE_CLOUD_PROJECT")
    )
