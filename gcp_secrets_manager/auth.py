...
def verify_credentials():
    ...
    try:
        ...
        if hasattr(credentials, 'service_account_email'):
            ...
            logger.info("Modo: Credenciales de Usuario Local (via 'gcloud auth application-default login')")

        return credentials, project_id

    except google.auth.exceptions.DefaultCredentialsError:
        logger.error("¡ERROR CRÍTICO! No se pudieron encontrar las credenciales de autenticación por defecto.")
        logger.error("Soluciones posibles:")
        logger.error("  1. (Para Desarrollo Local): Ejecuta en tu terminal -> gcloud auth application-default login")
        logger.error("  2. (Para Producción en GCP): Asegúrate de que el servicio tiene una Service Account con permisos.")
        raise